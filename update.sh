...
## 新增定时任务
add_cron () {
	for add_cron_list in $(diff $file_list_old $scripts_list_new | grep "+" | grep -v '@\|scripts.list' | sed 's/+//g'); do
		if [[ -n $add_cron_list ]]; then
			...
			if [[ -z $add_task_cron || -z $add_task_word ]]; then
				...
				if [[ $add_task_cron_min == 59 || $add_task_cron_min == 0 || $add_task_cron_min == 29 || $add_task_cron_min == 30 ]]; then
					echo -e "$add_task_cron task $add_task_name now\n" >> $file_crontab_user
				else
					echo -e "$add_task_cron task $add_task_name\n" >> $file_crontab_user
				fi
